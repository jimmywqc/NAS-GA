# # -*- coding: utf-8 -*-
# """
# Created on Thu Mar 24 15:59:31 2022
...
METRIC_OPS = [operator.__lt__, operator.__gt__] #这两个代表的含义A<B  A>B
...
class DEvol:
    ...
    def set_objective(self, metric):
        ...
        """
        #不是很理解 为什么要用operator os 之类的写代码 显得很复杂，操作也不是很直接
        if metric == 'acc':
            metric = 'accuracy'
    ...
    def _reproduce(self, pop, gen): 
        ...
        for _ in range(int(len(pop) * 0.95)):
            members.append(self._crossover(pop.select(), pop.select())) #child只有一个吗？ 最后gen不是会越来越少？
    ...
    def _evaluate(self, genome, epochs):
        ...
        self._record_stats(model, genome, loss, accuracy) #选出最优的 并记录
    ...
    def _evaluate_population(self, members, epochs, fitness, igen, ngen):
        ...
        for imem, mem in enumerate(members):
            ...
            res = self._evaluate(mem, epochs) #这里包含了记录最优模型的参数（估计是为了避免输出很多模型）
    ...
    def _print_evaluation(self, imod, nmod, igen, ngen):
        ...
        print(fstr.format(imod + 1, nmod, igen + 1, ngen)) #str.format 用来输出文字
...
class _Population(object): #用来做population更新的 
    ...
    def select(self): #轮盘赌的选择 这样写选择是不是不对
        dart = rand.uniform(0, self.s_fit)
        sum_fits = 0
        for i in range(len(self.members)):
            sum_fits += self.scores[i]
            if sum_fits >= dart:
                return self.members[i]  #这是返回一列数据还是一个数据
