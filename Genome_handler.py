# # -*- coding: utf-8 -*-
# """
# Created on Thu Mar 24 15:57:56 2022
...
class GenomeHandler:
    ...
    def __init__(self, max_conv_layers, max_dense_layers, max_filters,
        ...
        self.layer_params = {
            "active": [0, 1],
            "num filters": [2**i for i in range(3, filter_range_max)],#这里是不是可以改成（3，max_filters）
            ...
            "max pooling": list(range(3)) if max_pooling else 0,# 估计是想1D 2D 3D 但是decoding的时候没用
        }
    ...
    def convParam(self, i):
        ...
        return self.layer_params[key] #反馈一个list
    ...
    def mutate(self, genome, num_mutations):
        ...
        for i in range(num_mutations):
            ...
            if index < self.convolution_layer_size * self.convolution_layers:
                if genome[index - index % self.convolution_layer_size]: #指只要非0就执行if  这里是指非第一层conv
    ...
    def decode(self, genome):
        ...
        model = Sequential()  #这里还是顺序连接 没有跳跃性
        dim = 0
        offset = 0
        if self.convolution_layers > 0:
            #我觉得这一层应该追踪宽/高的最小值，并且跟新方式也不对
        ...
        for i in range(self.convolution_layers): 
            if genome[offset]:
                ...
                if max_pooling_type == 1 and dim >= 5:   
                    ...
                    dim = int(math.ceil(dim / 2)) #math.ceil 向上取整
