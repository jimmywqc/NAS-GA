...
import Devol_LSTM, Genome_handler_LSTM
import os
import tensorflow as tf
from keras.models import load_model

#max_dense_nodes>=16  max_filters>=8
genome_handler = Genome_handler_LSTM.GenomeHandler(max_lstm_layers=6, max_dense_layers=10,
                 max_dense_nodes=256, input_shape=x_train.shape[1:], n_classes=16)
devol = Devol_LSTM.DEvol(genome_handler)
_,average_acc,best_acc,acc = devol.run(dataset=dataset,num_generation=5,pop_size=5,epochs=50)
